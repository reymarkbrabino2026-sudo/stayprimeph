...
function GalleryTile({ src, alt, className }: { src?: string; alt: string; className?: string }) {
  return (
    <div className={`relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-rose-100 via-orange-50 to-stone-100 ${className ?? ""}`}>
      {isRenderableImage(src) ? <Image src={src!} alt={alt} fill sizes="(min-width:1024px) 50vw, 100vw" className="object-cover" /> : null}
    </div>
  );
}
...
export function PropertyGallery({ property }: { property: Property }) {
  return (
    <div className="grid gap-4 lg:grid-cols-[1.5fr_1fr]">
      <GalleryTile src={property.images[0]?.imageUrl} alt={property.title} className="min-h-[320px]" />
