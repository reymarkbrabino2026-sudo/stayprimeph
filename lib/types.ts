...
export interface Property { id: string; hostId: string; slug: string; title: string; description: string; address: string; city: string; country: string; virtualTourUrl?: string; bookingType?: ListingBookingType; pricePerNight: number; weekendPrice?: number; holidayPrice?: number; holidayDates?: string[]; seasonalRates?: SeasonalRate[]; cleaningFee?: number; securityDeposit?: number; currency?: string; bedrooms: number; bathrooms: number; maxGuests: number; propertyType: string; status: ListingStatus; rating: number; amenities: string[]; rules: string[]; createdAt: string; images: PropertyImage[]; discounts?: ListingDiscounts; rooms?: PropertyRoom[]; bookingPackages?: BookingPackage[]; latitude?: number; longitude?: number; barangay?: string; province?: string; zipCode?: string; preciseLocation?: boolean; }
