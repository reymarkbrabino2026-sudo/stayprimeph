...
export interface Property { id: string; hostId: string; slug: string; title: string; description: string; address: string; city: string; country: string; virtualTourUrl?: string; listingVideoUrl?: string; bookingType?: ListingBookingType; pricePerNight: number; weekendPrice?: number; holidayPrice?: number; holidayDates?: string[]; seasonalRates?: SeasonalRate[]; rateAdjustments?: ListingRateAdjustment[]; cleaningFee?: number; securityDeposit?: number; currency?: string; bedrooms: number; bathrooms: number; maxGuests: number; eventCapacity?: number; sleepingCapacity?: number; propertyType: string; privacyType?: string; status: ListingStatus; rating: number; amenities: string[]; rules: string[]; createdAt: string; images: PropertyImage[]; discounts?: ListingDiscounts; rooms?: PropertyRoom[]; bookingPackages?: BookingPackage[]; latitude?: number; longitude?: number; barangay?: string; province?: string; zipCode?: string; preciseLocation?: boolean; }
...
export interface AvailabilityBlock { id: string; propertyId: string; date: string; reason: AvailabilityBlockReason; note?: string; bookingPackageId?: string; bookingPackageName?: string; createdAt: string; }
