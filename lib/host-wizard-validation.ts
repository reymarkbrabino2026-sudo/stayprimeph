...
import { isEntirePlacePrivacyType } from "@/lib/host-wizard-steps";
...
export function canAdvanceFromStep(step: WizardStepId, draft: HostListingDraft) {
  switch (step) {
    ...
    case "rooms": return !isEntirePlacePrivacyType(draft.privacyType) || draft.rooms.length === 0 || draft.rooms.some((item) => item.active && item.name.trim() && item.capacity >= 1 && item.floor.trim());
