import { maskEmail, scrubMeta, scrubString } from "@/lib/privacy-scrubbing";

type LogMeta = Record<string, unknown>;

function write(level: "info" | "warn" | "error", event: string, meta: LogMeta = {}) {
  const payload = JSON.stringify({ level, event, ...scrubMeta(meta), timestamp: new Date().toISOString() });
...
export const logSanitizer = {
  maskEmail,
  sanitizeMeta: scrubMeta,
  sanitizeString: scrubString,
};
