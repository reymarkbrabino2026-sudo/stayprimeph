...
This plan defines the alerts required before public launch. It assumes Sentry, Vercel, Stripe, Resend, Cloudinary, Upstash, and the database provider are configured in their provider dashboards.
...
- Payment failure: Stripe webhook delivery failures, checkout session creation errors, dispute events, or elevated failed-payment rates.
...
| Stripe | Webhook delivery failure or checkout/payment failure spike | Immediate |
